...
# --- 📲 RAPPELS WHATSAPP ---
    ...
    st.header(f"📲 Centre de Rappels & Messages WhatsApp — {cellule_selected}")
    membres_totaux = obtenir_tous_les_membres_uniques(cell_data)
    ...
    col_stat1.metric("📱 Membres joignables (WhatsApp)", len(membres_avec_tel))
    col_stat2.metric("⚠️ Membres sans numéro", len(membres_sans_tel))

    st.subheader("✍️ Rédiger un message de rappel")
    ...
    message_personnalise = st.text_area("Texte du message :", value=msg_defaut, height=120)

    st.subheader("📤 Envoyer le message direct")

    if membres_avec_tel:
        membre_dest = st.selectbox("Sélectionner un destinataire :", [m["nom"] for m in membres_avec_tel])
        membre_sel = next(m for m in membres_avec_tel if m["nom"] == membre_dest)
        num_wa = nettoyer_numero(membre_sel["tel"])

        texte_encode = urllib.parse.quote(message_personnalise)
        lien_wa = f"https://wa.me/{num_wa}?text={texte_encode}"

        st.markdown(f"👉 **Envoyer à {membre_sel['nom']} ({membre_sel['tel']}) :**")
        st.markdown(f'<a href="{lien_wa}" target="_blank" style="background-color: #25D366; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; font-weight: bold;">📲 Ouvrir WhatsApp & Envoyer</a>', unsafe_allow_html=True)
    else:
...
# --- 📅 ÉVÉNEMENTS À VENIR ---
    ...
    if events:
        for idx, evt in enumerate(events):
            ...
            # Envoi direct de rappel pour cet événement
